...
| 5 | Disk watch | ✅ folded into `install.sh backup` (warns ≥ 85%) |
...
- [ ] **Backup cron installed** and the cron daemon is running
  (`crontab -l`, `pgrep -x cron`). Do a manual `./install.sh backup` and confirm a
...
1. Confirm `main` is deployed and healthy:
   ```bash
   ./install.sh update                 # pull :latest → restart → health-check → prune
...
```bash
./install.sh update        # pull :latest, restart, health-check, prune old images
```

**Backups**
- Nightly via cron; on demand: `./install.sh backup`.
- **Restore:**
  ```bash
...
crash-loops the container. `install.sh update` prunes on every deploy; if it still fills:
```bash
...
**Rollback**: `install.sh update` keeps the newest few per-commit image tags. Re-point
...
- **Backups cover the DB, not uploads**: the `portal-uploads` volume (logos,
  images) isn't in `install.sh backup`; add it if those matter.
