...
| Task | Command | Notes |
|---|---|---|
| Update to latest release | `./update.sh` | Checkout + image + `.env` reconcile; restarts only on change. Cron-safe. |
| Unconditional redeploy | `./deploy.sh` | Pull `:latest`, restart, health-check, verify migrations, prune. |
| Nightly backup | `backup.sh` (cron) | `pg_dump` → `/backups`, 14-day rotation. |
| Restore | `./restore.sh [dump]` | **Destructive**; stop the app first. |

Reference deploy host crontab (root; `MAILTO` as a variable line so cron
mails the output):

(`/opt/unifi-captiveportal` stands for your install directory - the repo can live anywhere; the scripts locate themselves.)

```cron
MAILTO=ops@example.com
0 3 * * *  /opt/unifi-captiveportal/backup.sh
30 4 * * * GIT_TERMINAL_PROMPT=0 /opt/unifi-captiveportal/update.sh
@reboot    GIT_TERMINAL_PROMPT=0 /opt/unifi-captiveportal/update.sh
```

The `@reboot` line brings the stack up on the **latest release** after a host
restart (`update.sh` waits up to 5 minutes for the Docker daemon, so early
firing is safe); the nightly line covers everything else, including plain
container restarts, within a day.

The `GIT_ASKPASS` helper is only needed when the host stores no git
credentials (it reads a root-only token file at runtime, the token never
lands in the crontab, the remote URL, or the script). Hosts with a cached
credential helper run plain `update.sh`. See README "Backups & health" →
*Updates* for details and the CI-shares-the-daemon retag quirk.

## Disaster recovery, RTO / RPO

- **RPO (max data loss): ≤ 24 h.** `backup.sh` runs nightly (`pg_dump`, gzip,
...
- **RTO (time to restore): ~minutes.** Recovery is `restore.sh` (resets schema,
  applies the dump with `ON_ERROR_STOP`) then `docker compose up -d`. Migrations
  run on container boot.

**Full-host recovery procedure:**
1. Reinstall Docker + clone the repo; run `./setup.sh` (it creates `.env` and
...
3. Restore the newest DB dump: `./restore.sh backups/portal-<newest>.sql.gz`.
...
## Branch protection (GitHub repo settings, not in git)

`main` and `develop` each require these status checks green before a PR
can merge — no required-review count, just the checks. GitHub Actions
contexts are bare job names, not the old Gitea `Workflow / job` style:
`test`, `trivy-scan`, `gitleaks`, `semgrep` (both branches), plus
`security/snyk (blakey108)` — Snyk's native GitHub App check — on `main`
only. `nightly` stays unprotected by design (fast-iteration exception).
Requires GitHub Pro on this private repo (branch protection isn't available
for private repos on the free plan). Configure under Settings → Branches →
Branch protection rules on
https://github.com/Blake-DK/unifi-captiveportal, or via
`gh api repos/Blake-DK/unifi-captiveportal/branches/<branch>/protection`.
...
- **Disk**: CI leaves ~1.4 GB per image; `deploy.sh`/`update.sh` prune, but a
...
3. `docker compose up -d` (or `./deploy.sh`, which pulls, restarts and
...
publishing — `deploy.sh` now bounces it on every deploy.
...
Interactive `update.sh` runs offer to run it while the feature is
...
- Single container = no rolling deploy; `update.sh` restart is a brief blip.
