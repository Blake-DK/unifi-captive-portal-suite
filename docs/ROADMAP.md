...
- [x] **Dependency audit in CI**: `npm audit --omit=dev --audit-level=critical`
      ...
      2026-07-06.)*
- [x] **Scheduled DB backup + restore runbook**: `backup.sh` (nightly
      `pg_dump` → `backups/`, 14-day rotation, atomic write, disk warning) plus
      `restore.sh` (newest-or-named dump, schema reset, `ON_ERROR_STOP`,
...
- [x] **A-lite — deploy wiring** (2026-07-12). `docker-compose.yml` gains a
  ...
  gets; `deploy.sh` pulls/restarts/health-checks both containers when the
...
- [x] **A-lite — hardening pass** (2026-07-12, after a full review of the
  ...
  back-out); `deploy.sh` fails closed on a `compose config` error and protects
  both containers' images from pruning; `setup.sh` re-runs preserve `split` in
