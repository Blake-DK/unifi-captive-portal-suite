...
// The inbound-exposure surface: static destination-NAT port forwards (admin
// configured) plus dynamic UPnP/IGD leases (opened by clients themselves).
// Read-only — the UPnP Inspector and threat exposure analysis both consume
// listPortForwards(); the dynamic UPnP lease table is best-effort (see below).

export type UniFiPortForward = {
  _id?: string;
  name?: string;
  enabled?: boolean;
  pfwd_interface?: string; // "wan" | "wan2" — which uplink the forward listens on
  fwd?: string; // internal target IP the traffic is NAT'd to
  fwd_port?: string; // internal port
  dst_port?: string; // external (WAN-facing) port, the one exposed to the internet
  proto?: string; // "tcp" | "udp" | "tcp_udp"
  src?: string; // "any" or a CIDR restricting who may reach the forward
  log?: boolean;
};

/** Admin-configured static port forwards (destination NAT) — GET /rest/portforward. */
export async function listPortForwards(): Promise<UniFiPortForward[]> {
  const { cfg } = await ensureSession();
  const res = await unifiRequest<{ data: UniFiPortForward[] }>(`/api/s/${cfg.unifiSite}/rest/portforward`);
  return res.data ?? [];
}

/** One dynamic UPnP/IGD or NAT-PMP lease a client opened on the gateway. Fields
 * are kept loose because the endpoint shape varies across Network builds (and
 * may not exist at all — see listUpnpMappings). */
export type UniFiUpnpMapping = {
  proto?: string;
  ext_port?: number | string;
  int_port?: number | string;
  int_ip?: string; // the LAN device that requested the mapping
  description?: string;
  duration?: number;
} & Record<string, unknown>;

export type UpnpProbe = { path: string; ok: boolean; count: number; error?: string; sampleKeys?: string[] };

/**
 * Best-effort read of the live UPnP/IGD lease table. UniFi does not expose this
 * through a single documented REST endpoint and older/leaner builds omit it
 * entirely, so probe the known candidates like findFirewallZones does and
 * degrade to an empty list with a probe log rather than throwing. `available`
 * is false when nothing answered — the caller shows "not exposed by this
 * controller" instead of an empty-but-implies-none table.
 */
export async function listUpnpMappings(): Promise<{ mappings: UniFiUpnpMapping[]; available: boolean; probes: UpnpProbe[] }> {
  const { cfg } = await ensureSession();
  const candidates = [
    `/api/s/${cfg.unifiSite}/stat/upnp`,
    `/v2/api/site/${cfg.unifiSite}/upnp`,
  ];
  const probes: UpnpProbe[] = [];
  for (const path of candidates) {
    try {
      const res = await unifiRequest(path);
      const list = v2List<UniFiUpnpMapping>(res);
      probes.push({ path, ok: true, count: list.length, sampleKeys: list[0] ? Object.keys(list[0]).slice(0, 12) : [] });
      // A path that answers 200 with an array (even empty) is the real endpoint:
      // treat it as authoritative and stop probing.
      return { mappings: list, available: true, probes };
    } catch (e) {
      probes.push({ path, ok: false, count: 0, error: (e instanceof Error ? e.message : String(e)).slice(0, 160) });
    }
  }
  return { mappings: [], available: false, probes };
}
